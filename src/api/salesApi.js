...
export const salesApi = {
  // TODO-019 (구현 완료): 기간 요약은 summary 객체로만 조회한다. QA: 실DB 집계와 period/date-range 오류 응답을 확인한다.
  ...
  // TODO-020 (구현 완료): 월별 차트 데이터는 year 파라미터로 조회한다. QA: 허용 연도 경계와 0-fill 월을 확인한다.
  getMonthly({ year }) {
    return apiClient.get(API_ENDPOINTS.salesMonthly, { params: { year } });
  },

  // TODO-021 (구현 완료): daily 객체와 time-slots 배열을 별도 메서드로 유지한다. QA: 30/60분, 빈 시간대, 잘못된 날짜를 확인한다.
  getDaily({ from, to } = {}) {
